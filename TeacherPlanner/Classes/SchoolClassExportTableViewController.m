//
//  SchoolClassExportTableViewController.m
//  TeacherPlanner
//
//  Created by Oliver Klemenz on 21.01.15.
//
//
...
@interface SchoolClassExportTableViewController ()
...
@implementation SchoolClassExportTableViewController
...
- (instancetype)initWithStyle:(UITableViewStyle)style {
    ...
    if (self) {
        self.name = NSLocalizedString(@"School Classes", @"");
        self.subTitle = NSLocalizedString(@"Exports", @"");
        self.editable = YES;
...
- (NSString *)subTitle {
    return [NSString stringWithFormat:@"%@ - %@", NSLocalizedString(@"Exports", @""), self.schoolYear.name];
}
...
- (void)refresh {
    ...
    [content enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if ([fileName hasSuffix:[NSString stringWithFormat:@".%@", kClassExtension]] ||
            [[fileName pathExtension] isEqualToString:kCSVExtension]) {
            ...
            if (fileAttributes) {
                ...
                [creationByExports addObject:@{ @"filePath" : exportPath,
                                                @"name" : [[fileName stringByDeletingPathExtension] stringByDeletingPathExtension],
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([[application[@"extension"] lowercaseString] isEqual:kZIPExtension]) {
        cell.imageView.image = [[UIImage imageNamed:@"zip"] tintImage:[Configuration instance].highlightColor];
    } else if ([[application[@"extension"] lowercaseString] isEqual:kCSVExtension]) {
        cell.imageView.image = [[UIImage imageNamed:@"csv"] tintImage:[Configuration instance].highlightColor];
    } else if ([[application[@"extension"] lowercaseString] isEqual:kXLSExtension]) {
        cell.imageView.image = [[UIImage imageNamed:@"xls"] tintImage:[Configuration instance].highlightColor];
    }
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        [Common showDeletionConfirmation:self okHandler:^{
            ...
            if ([Utilities deletePath:export[@"filePath"]]) {
                [Utilities deletePath:[export[@"filePath"] stringByAppendingPathExtension:kZIPExtension]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self showImportPopup:^{
        [ShareUtilities showSchoolClassImportOptions:self handler:^(NSInteger option) {
            if (option == kActionSheetImportSchoolClassCancel) {
                return;
            }
            NSDate *date = self.createDate[indexPath.section];
            NSDictionary *export = [self.exportsByCreateDay[date] objectAtIndex:indexPath.row];
            if (![SchoolClass importSchoolClass:export[@"filePath"] password:nil]) {
                [ShareUtilities showPasswordEntry:self handler:^(NSString *password) {
                    if ([SchoolClass importSchoolClass:export[@"filePath"] password:password]) {
                        [Common showMessage:self title:NSLocalizedString(@"School Class Import", @"") message:NSLocalizedString(@"School class successfully imported.", @"") okButtonTitle:nil okHandler:nil];
                    } else {
                        [Common showMessage:self title:NSLocalizedString(@"School Class Import", @"") message:NSLocalizedString(@"Error during school class import. Wrong Password?", @"") okButtonTitle:nil okHandler:nil];
                    }
                }];
            } else {
                [Common showMessage:self title:NSLocalizedString(@"School Class Import", @"") message:NSLocalizedString(@"School class successfully imported.", @"") okButtonTitle:nil okHandler:nil];
...
- (void)showImportPopup:(void (^)(void))okHandler cancelHandler:(void (^)(void))cancelHandler {
    [Common showConfirmation:self title:NSLocalizedString(@"Confirm School Class Import", @"")
                     message:NSLocalizedString(@"Do you really want to import the school class?", @"")
